...
const flattenAction = async ({ files, logFunction, warnFunction }, { solidity, config, hooks }) => {
    ...
    const dependencyGraph = await buildDependencyGraph(rootPaths.toSorted(), // We sort them to have a deterministic order
    config.paths.root, readSourceFileFactory(hooks));
    let flattened = "";
...
function formatSourceName(file) {
    return file.type === ResolvedFileType.NPM_PACKAGE_FILE
        ? file.inputSourceName
        : file.inputSourceName.substring(file.package.inputSourceNameRoot.length + 1);
}
...
function getSortedFiles(dependencyGraph) {
    const sortedFiles = [];
    // Helper function for sorting files by sourceName, for deterministic results
    const sortBySourceName = (files) => {
        return Array.from(files).sort((f1, f2) => f1.inputSourceName.localeCompare(f2.inputSourceName));
    };
    // Depth-first walking
    const walk = (files) => {
        for (const file of files) {
            ...
            const dependencies = sortBySourceName(Array.from(dependencyGraph.getDependencies(file)).map((d) => d.file));
            walk(dependencies);
        }
    };
